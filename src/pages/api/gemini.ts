...
export default async function handler(
  ...
  try {
    const fileName = `${name.trim().toLowerCase().replace(/\s+/g, "_")}}_kundali.json`
    const filePath = path.join(process.cwd(), "public", fileName);

    if (!fs.existsSync(filePath)) {
      return res.status(404).json({ message: "Kundali file not found" });
    }

    const kundaliFileData = fs.readFileSync(filePath, "utf-8");
    console.log("Kundali file data:", kundaliFileData);
    const prompt = `
